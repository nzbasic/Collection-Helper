...
const migrateFilters = async () => {
  const oldFilter = "(async () => {\n\tlet topSets = (await axios.get('https://osutracker.com/api/stats/farmSets')).data\n\tlet filtered = beatmaps.filter(beatmap => topSets.includes(beatmap.setId.toString()))\n\tresolve(filtered)\n})()"
  const farm = await database.get("SELECT * FROM filters WHERE filter = ?", [oldFilter]);

  if (farm) {
    await database.run("UPDATE filters SET filter = ? WHERE filter = ?", [farmFilter.filter, oldFilter])
  }
}
