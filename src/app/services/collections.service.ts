...
export class CollectionsService {
  ...
  async removeCollections(names: string[]): Promise<void> {
    this.collections = (await axios.post("http://localhost:7373/collections/remove", names)).data
  }

  async addCollection(name: string): Promise<void> {
    this.collections = (await axios.post("http://localhost:7373/collections/add", {name: name})).data
  }

  async mergeCollections(names: string[]): Promise<void> {
    this.collections = (await axios.post("http://localhost:7373/collections/merge", names)).data
  }

  async renameCollection(oldName: string, newName: string): Promise<void> {
    this.collections = (await axios.post("http://localhost:7373/collections/rename", { oldName: oldName, newName: newName })).data
  }

  async addMaps(hashes: string[], name: string): Promise<Collection> {
    this.collections = (await axios.post("http://localhost:7373/collections/addMaps", { name: name, hashes: hashes })).data
  ...
  async removeMaps(hashes: string[], name: string): Promise<Collection> {
    this.collections = (await axios.post("http://localhost:7373/collections/removeMaps", { name: name, hashes: hashes })).data
    return this.collections.collections.find(collection => collection.name == name)
  }
}
